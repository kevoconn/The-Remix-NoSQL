...
module.exports = {
  ...
  deleteUser(req, res) {
    User.findOneAndDelete({ _id: req.params.courseId })
      .then((user) => (!user ? res.status(404).json({ message: "No user with that ID" }) : Student.deleteMany({ _id: { $in: user.students } })))
      .then(() => res.json({ message: "User deleted!" }))
      .catch((err) => res.status(500).json(err));
  },
  // Update a course
  updateUser(req, res) {
    User.findOneAndUpdate({ _id: req.params.courseId }, { $set: req.body }, { runValidators: true, new: true })
